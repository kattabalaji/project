function [precision, recall, cmat] = knns(numOfReturnedImgs, dataset, queryImageFeatureVector, metric)
...
for k=1:numel(svmModel)
    ...
    knn{k} = ClassificationKNN.fit(dataset(idx,:), g(idx),'Distance','seuclidean');

    %# test
    predTest(:,k) = knn{k}.predict(dataset(testIdx,:)); % matlab native svm function
end
...
for k = 1:numel(knn{k})
    %# test
    predQueryImg(:, k) = knn{k}.predict(queryImageFeatureVector); % queryImage = x.jpg, row=x from dataset
end
