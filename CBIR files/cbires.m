...
function cbires_OpeningFcn(hObject, ~, handles, varargin)
...
function varargout = cbires_OutputFcn(~, ~, handles)
...
function popupmenu_NumOfReturnedImages_Callback(hObject, ~, handles)
...
function btnExecuteSVM_Callback(~, ~, handles)
...
% call  function passing as parameters the numOfReturnedImgs, queryImage and the dataset
[~, ~, ~] = knns(numOfReturnedImgs, handles.imageDataset.dataset, handles.queryImageFeature, metric);
[~, ~, cmat] = CNN(numOfReturnedImgs, handles.imageDataset.dataset, handles.queryImageFeature, metric);

% plot confusion matrix
opt = confMatPlot('defaultOpt');
opt.className = {
    'Flowers', 'Beach', 'Monuments', ...
...
function btnPlotPrecisionRecall_Callback(~, ~, handles)
...
for k = 1:15
    ...
    [precision, recall] = svmclassify(numOfReturnedImgs, database, queryImageFeature, metric);
...
function btnSelectImageDirectory_Callback(hObject, ~, handles)
...
function btnCreateDB_Callback(~, ~, handles)
    ...
    for k = 1:totalImages
        ...
        if ( (num_of_jpg_images - jpg_counter) > 0)
            ...
            if ( strcmpi( imgInfoJPG.Format, 'jpg') == 1 )
                ...
                [~, name, ~] = fileparts( fullfile( handles.folder_name, jpg_files(jpg_counter+1).name ) );
        ...
        elseif ( (num_of_png_images - png_counter) > 0)
            ...
            if ( strcmpi( imgInfoPNG.Format, 'png') == 1 )
                ...
                [~, name, ~] = fileparts( fullfile( handles.folder_name, png_files(png_counter+1).name ) );
                image = imresize(image, [384 256]);
            end
        ...
        elseif ( (num_of_bmp_images - bmp_counter) > 0)
            ...
            if ( strcmpi( imgInfoBMP.Format, 'bmp') == 1 )
                ...
                [~, name, ~] = fileparts( fullfile( handles.folder_name, bmp_files(bmp_counter+1).name ) );
                image = imresize(image, [384 256]);
            end
...
function btn_LoadDataset_Callback(hObject, ~, handles)
